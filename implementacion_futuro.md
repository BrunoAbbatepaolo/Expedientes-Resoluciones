...
- ✅ **2.1** (autorización entre oficinas) — resuelto para `Expediente` (validación de permiso + pertenencia a oficina); resuelto para `Resolucion` a nivel de permiso (ver limitación abajo). Detalle:
  ...
  - **Limitación conocida:** `Resolucion` no tiene columna `oficina_id` ni relación con `Oficina` en el esquema actual (solo se vincula a `VistaExpedientes` por `numero_exp`, sistema externo "MITIV"). Por eso el chequeo de resoluciones valida permiso pero **no** pertenencia a oficina — cualquier usuario con `resolucion_editar` puede editar resoluciones de cualquier oficina. Cerrar esto del todo requeriría agregar una columna `oficina_id` a `resoluciones` (migración + backfill), fuera de alcance de este punto.
  - Validado con un script standalone (`bootstrap/app.php` + tinker manual) contra el contenedor de prueba: usuario de la oficina dueña del expediente → permitido; usuario de otra oficina → bloqueado con HTTP 403.
- ✅ **1.4 + 1.5 + 2.6** (esquema y uso real de `pases`) — resuelto parcialmente (ver limitación). Detalle:
  ...
  - **Limitación conocida (decisión de producto, no técnica):** al registrar el pase, `Expediente::oficina_id` (que determina en qué oficina aparece el expediente en `Expedientes::getExp()`) **no se reasigna** a la oficina de destino. Hoy un expediente sigue apareciendo en el listado de la oficina de origen (como "egresado") pero nunca aparece en el listado de la oficina de destino — no hay una bandeja de "entrantes" del lado del receptor. Cambiar esto implica redefinir qué significa "mi oficina" en `getExp()` y afecta las pantallas principales de listado; no lo hice porque es una decisión de negocio (no solo un bug) y no pude probarlo contra datos reales de `expedientes`/`oficinas` (no existen en ningún entorno disponible). Queda pendiente de decisión del usuario.
